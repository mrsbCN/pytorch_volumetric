...
class Chain:
    ...
    def forward_kinematics(self, th, frame_indices: Optional = None):
        ...
        B_base = _initial_world_accumulator_matrix.shape[0]

        # Batch compatibility check
        if B_base > 1 and B_th > 1 and B_base != B_th:
            raise ValueError(f"Batch size of base_transform ({B_base}) and th ({B_th}) must be equal or one of them must be 1 for broadcasting.")

        # frame_transforms will store the computed world-coordinate matrices (tensors) for memoization
        # It maps integer frame indices to (Batch, 4, 4) tensors. Batch size is max(B_base, B_th)
        ...
        for requested_frame_idx_item in frame_indices_list:
            ...
            for i in range(start_from_idx, len(path_to_requested_frame)):
                ...
                if jnt_type != 0: # If not a fixed joint
                    ...
                    # current_fk_matrix could be (B_base,4,4) or (max(B_base,B_th),4,4)
                    # jnt_transform could be (B_th,4,4)
                    # PyTorch matmul broadcasting: (J,N,M) @ (K,M,P) -> (JorK, N,P) if J or K is 1
                    # Here: (B_cur,4,4) @ (B_th,4,4) where B_cur is B_base or max(B_base,B_th)
